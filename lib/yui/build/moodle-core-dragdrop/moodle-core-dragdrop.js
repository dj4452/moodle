...
Y.extend(DRAGDROP, Y.Base, {
    ...
    global_start_keyboard_drag: function(e, draghandle, dragcontainer) {
        ...
        droptargets.each(function(node) {
            ...
            if (node.drop && node.drop.inGroup(this.groups) && node.drop.get('node') !== dragcontainer && node.next(classname) !== dragcontainer) {
                ...
                for (i = 0; i < elementgroups.length; i++) {
                    for (j = 0; j < this.groups.length; j++) {
                        if (elementgroups[i] === this.groups[j]) {
                            if (node == dragcontainer || node.next(classname) === dragcontainer  || node.get('children').item(0) == dragcontainer) {
                                validdrop = false;
                            } else {
